...
public class BodyParser : RouterMiddleware {
    ...
    public class func parse(message: BlueSocketReader, contentType: String?) -> ParsedBody? {
        ...
        if let parser = parserArchiver[contentType] {
            return parseImplement(message, parser: parser)
        }else if contentType.hasPrefix("text/"){
            return parseImplement(message, parser: parserArchiver["text"]!)
        }
    ...
    ///
    private class func parseImplement(message: BlueSocketReader, parser: ((NSMutableData) -> ParsedBody?)) -> ParsedBody? {
        do {
