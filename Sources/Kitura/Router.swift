...
public class Router {
    ...
    /// - Parameter options: (Optional) A Dictionary of options to be used by the
    ///                     template engine.
    /// - Returns: The content generated by rendering the template.
    /// - Throws: Any error thrown by the Templating Engine when it fails to
    ///          render the template.
    internal func render(template: String, context: [String: Any], options: [String: Any]? = nil) throws -> String {
        ...
        if let opt = options {
            return try templateEngine.render(filePath: absoluteFilePath, context: context, options: opt)
        } else {
