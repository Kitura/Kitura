...
class KituraTests: XCTestCase {
    func testResponseOK() {
        let request = HTTPRequest(method: .get, target:"/echo", httpVersion: HTTPVersion(major: 1,minor: 1), headers: HTTPHeaders())
    ...
    func testHello() {
        let request = HTTPRequest(method: .get, target:"/helloworld", httpVersion: HTTPVersion(major: 1,minor: 1), headers: HTTPHeaders())
        let resolver = TestResponseResolver(request: request, requestBody: Data())
        var router = Router()
